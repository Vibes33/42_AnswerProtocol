...
// EconomyConfig: every defeated monster gives its full gold to each participant.
...
// SellPrice is what a merchant pays for an item. Items with price 0 cannot be sold.
func (e EconomyConfig) SellPrice(it *Item) int {
	return int(float64(it.Price) * e.SellRatio)
}

// BuyPrice is the shop entry's own price when set, otherwise the item's base price.
...
// LevelingConfig uses a linear curve: reaching level L+1 costs XPBase + XPPerLevel * (L - 1).
...
// CombatConfig: a combat turn offers MaxEquippedMoves moves plus two fixed actions, rest and use item.
